...
export class TaskComponent {
  ...
  constructor(private fb: UntypedFormBuilder, private authService: AuthService,private http: HttpClient) {
    const blank = {} as Task;
  ...
  fetch(cb: (i: any) => void) {
    ...
    if(id) {
      this.http.get(`${environment.apiUrl}/gettasks?id=${id}`).subscribe((val) => {
        cb(val);
      })
    }
