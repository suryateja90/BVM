  ...
  implements OnInit
{
  ...
  loadAll() {
    const getMyEvents$ = this.auth.get('/getTeacherEvents');
    ...
    forkJoin([getMyEvents$, getDemoEvent$]).subscribe({
      next: ([myEventsResponse, demoEventResponse]) => {
        ...
        const demoEvents = demoEventResponse?.filter((e:any) => (e.status == 'completed'  || e.status == 'approved'));
  ...
  loadCalender(val:any,demoEvents:any) {
    this.filters = [];
      ...
      const formattedEvents: EventInput[] = val.map((event: any) => {
        ...
        let fixClass =  'fc-event-primary';
        if(event.status === 'approved') {
          fixClass = 'fc-event-success'
        }
...
setMode() {
        ...
        const obj =
          {
            ...
            "end_date": null,
            "teacher_id": this.calendarData?.teacher_id,
  ...
  filterEvent(element: string[]) {
    const list = this.tempEvents?.filter((x) =>
      element.map((y?: string) => y).includes(x['status'])
    );
  ...
  addDemoEvent() {
    ...
    this.subs.sink = dialogRef.afterClosed().subscribe((result) => {
      if (result === 'submit') {
        ...
        this.auth.post('/add/demoevent', this.calendarData).subscribe((val) => {
          if(val) {
            this.addCusForm.reset();
            this.showNotification(
              'snackbar-success',
  ...
  doDeleteDemo() {
    this.calendarData = this.calendarService.getDialogData();
    this.auth.delete(`/delete/demoevent?id=${this.calendarData.id}`,).subscribe({
      next: (v) => {
        this.showNotification(
          'black',
          'Deleted Record Successfully...!!!',
