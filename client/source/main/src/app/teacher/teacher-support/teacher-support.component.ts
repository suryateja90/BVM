...
@Component({
  ...
  styleUrls: ['./teacher-support.component.scss']
})
export class TeacherSupportComponent {
  ...
  displayedColumns: string[] = ['raiseby_name', 'description', 'closure', 'done', 'actions'];
  ...
  loadData() {
    this.auth.get(`/get/support?id=${this.user.id}&role=${this.user.role}`).subscribe({
      next: (result: any) => {
  ...
  buildForm() {
    ...
    this.supportForm = this.fb.group({
      ...
      raiseby_id: [currentUser.id, [Validators.required]],
